...
const texts = {
  en: {
    signInTitle: 'Welcome to Sybau Picture',
    signInSubtitle: 'Sign in with Google to start creating amazing AI-powered content',
    signInWithGoogle: 'Continue with Google',
    signingIn: 'Signing in...',
    features: {
      feature1: 'AI-Powered Image Generation',
      feature2: 'High-Quality Results',
      feature3: 'Easy to Use Interface',
      feature4: 'Secure & Private'
    }
  },
  zh: {
    signInTitle: '欢迎使用 Sybau Picture',
    signInSubtitle: '使用 Google 账户登录，开始创建令人惊艳的 AI 内容',
...
export default function SignInPage() {
  ...
  useEffect(() => {
    async function checkSession() {
      ...
      if (session) {
        const targetPath = language === 'zh' ? '/zh' : '/'
  ...
  const handleGoogleSignIn = async () => {
    try {
      console.log('Google登录按钮被点击，语言:', language)
      const result = await signIn('google', {
        callbackUrl: language === 'zh' ? '/zh' : '/',
        redirect: true
      })
